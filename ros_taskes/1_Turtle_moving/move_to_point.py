...
def main():
    ...
    #стартовые точки
    start_x, start_y = 0, 0 #pole 11 na 11.
    turtle_controller.grid_teleport(start_x*10, start_y*10)
    
    road = turtle_controller.a_star_path((start_x, start_y),(goal_x*10, goal_y*10),blocks)
    turtle_controller.set_pen_color(0,255,0,1,0)
    for node in road:
